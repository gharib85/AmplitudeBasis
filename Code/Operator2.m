...
testPair[pairlist_,current_,remain_,tensor_,other_,gotbasis_,ampbasis_,num_,OptionsPattern[]]:=Module[{templist=pairlist,templist2,tempcurrent=current,tempcurrent2,tempremain=remain,tempremain2,newope,newcoor,newbasis=gotbasis,lbasis},
If[tensor[[0]]===List,Do[ltensor=i;(*Print[{pairlist,current\[LeftDoubleBracket]i,1\[RightDoubleBracket],current\[LeftDoubleBracket]i,2;;\[RightDoubleBracket],tensor\[LeftDoubleBracket]i\[RightDoubleBracket],newbasis,OptionValue[AdlerZero]}];*)newbasis=testPair[pairlist,current[[i,1]],current[[i,2;;]],tensor[[i]],other[[i]],newbasis,ampbasis,num,doubleindex->OptionValue[doubleindex],AdlerZero->OptionValue[AdlerZero]],{i,Length[tensor]}];Throw[newbasis]];If[OptionValue[AdlerZero]===False,lbasis=Length[ampbasis],lbasis=OptionValue[AdlerZero]];
Do[
Do[(*Print["start=",i,j];*)
...
If[Length@tempcurrent2==0==Length@tempremain2,(*Sow[templist2];Return[];*)newope=other tensortooper[TensorContract[tensor,DoubleContract[templist2,OptionValue[doubleindex]]]];(*Print["check=",newope];*)
(*Print["check=",num];
Print["check=",ampbasis];Print["check=",gotbasis];*)newcoor=FindCor[reduce[Amp[#]/.{Subscript[D, n_][\[Rho]_]^2:>0},num],ampbasis]&/@Append[(*gotbasis*)newbasis,newope];(*Print["check=",newcoor];*)If[MatrixRank[newcoor]>Length[(*gotbasis*)newbasis],newbasis=Append[(*gotbasis*)newbasis,newope];lnewbasis=Length[newbasis];(*Print[newope];*)If[Length[newbasis]===lbasis,(*Print["enough"];*)Throw[newbasis](*,Print["not enough"]*)]];(*Print["return=",newbasis];*)Return@Return@Return[newbasis]];
...
testLorBasis[tensor_,other_,indgroup_,state_,k_,OptionsPattern[]]:=Module[{result,input,ampbasis,lbasis,coor,gen1,gen2,num=Length@state},(*{state,k}=tensor2state[tensor,other,num];Print[{state,k}];*)ampbasis=SSYT[state,k,OutMode->"amplitude"];lbasis=If[OptionValue[AdlerZero]===False,Length[ampbasis],SoftBlocks[state,k,OptionValue[AdlerZero]]//Length];Print["find ",Dynamic[lnewbasis],"/",lbasis," basis"];If[tensor[[0]]===List,Print["find ",Dynamic[ltensor],"/",Length[tensor]," tensors"];input=Reverse@SortBy[#,Length@#&]&/@indgroup;result=Catch[testPair[{},input,{},tensor,other,{},ampbasis,num,doubleindex->OptionValue[doubleindex],AdlerZero->lbasis]],input=Reverse@SortBy[indgroup,Length@#&];result=Catch[testPair[{},input[[1]],input[[2;;]],tensor,other,{},ampbasis,num,doubleindex->OptionValue[doubleindex],AdlerZero->lbasis]]];Return[<|"LorBasis"->result,"Trans"->(FindCor[reduce[Amp[#],num],ampbasis]&/@result)|>];If[OptionValue[AdlerZero]===False,Return[result],Print[result];coor=FindCor[reduce[Amp[#],num],ampbasis]&/@result;gen1=LinearSolve[coor\[Transpose],FindCor[YPermute[#.ampbasis,Cycles[{OptionValue[AdlerZero][[{1,2}]]}],num],ampbasis]]&/@coor;
